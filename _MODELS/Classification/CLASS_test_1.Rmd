...
DATA <- read.csv("mimic_test_1.csv")
mimic <- DATA
```
...
a <- bind_rows(RANDON_FOREST, KLAB, RANGER) %>%
  group_by(model) %>% 
  roc_curve(LOS_GROUP_ICU, .pred_A:.pred_C) %>%
  autoplot()
...
a +  labs(title = "Test 1",
          ...
          caption = "A: (LOS_ICU <= 3), B: (3 < LOS < 5), C: (LOS >= 5)")
...
```{r}
ggsave(file="test1.png", width=12, height=3, dpi=600)
```
