...
- I was using the wrong dataset for PCR analysis, this explain the wrong values in the table.
...
```{r}
training <- DATA_urgent_3$LOS_ICU %>% createDataPartition(p = 0.8, list = FALSE)

train.data <- DATA_urgent_3[training,]
test.data <- DATA_urgent_3[-training,]
```
...
```{r}
plot(predictions_pls)
```

```{r}
save.image("REG_Urgenve_v5_0.RData")
```
